...
**Version 1.1 · June 2026 · ReviveIQI**
...
| # | Question | Status |
|---|---|---|
| OQ-1 | Do we want a Slack integration for follow-up alerts in v1.0? | ✅ Yes — include in v1.0 |
| OQ-2 | Should the free tier include Gmail only, or both providers? | ✅ Both Gmail + Outlook on free tier |
| OQ-3 | Token storage: encrypt access/refresh tokens at rest in TiDB? | ✅ Yes — encrypt at rest. AES-256 via Node crypto before DB write |
| OQ-4 | Should workspace members see each other's contact names, or anonymized? | ✅ Fully visible — all members see all contacts on shared board |
| OQ-5 | Weekly digest: triggered by cron or event-based? | ✅ Cron — every Monday 8am EST (same pattern as MyCareerIQ) |
| OQ-6 | Domain: inboxiq.reviveiqi.com subdomain or standalone inboxiq.io? | ✅ inboxiq.reviveiqi.com — stays in ReviveIQI suite |
...
*InboxIQ PRD v1.1 · ReviveIQI · Confidential*

---

## 15. Decisions Log

All open questions resolved — PRD v1.1 ready for Phase 1 build.

| # | Decision | Rationale |
|---|---|---|
| OQ-1 | Slack in v1.0 | B2B sales teams live in Slack. Follow-up alerts where they already work removes friction from the core loop. |
| OQ-2 | Both providers on free | Broader free adoption. Outlook is common in enterprise — blocking it on free limits reach into the primary target market. |
| OQ-3 | Encrypt tokens at rest | OAuth tokens grant inbox access. AES-256 encryption before TiDB write is non-negotiable for B2B trust. |
| OQ-4 | Fully visible to all members | Sales teams share contact context. A rep should know their teammate talked to a prospect last week. Full visibility = better collaboration. |
| OQ-5 | Cron (Monday 8am EST) | Simple, predictable, consistent with MyCareerIQ daily cron pattern. Event-based can be added in v1.1 once we know what signals matter. |
| OQ-6 | inboxiq.reviveiqi.com | Keeps the product in the ReviveIQI suite for v1.0. Standalone domain (inboxiq.io) available as future option if InboxIQ needs independent GTM. |

**PRD version bumped to v1.1** — all decisions locked, Phase 1 build can begin.
